...
class ReservasiController extends ChangeNotifier {
  ...
  /// Expire reservasi - untuk testing fitur pickup
  Future<bool> expireReservasi(String reservasiId) async {
    return _updateStatus(reservasiId, 'expire');
  }

  // ==================== PICKUP METHODS ====================

  /// Schedule pickup - Admin jadwalkan penjemputan
  Future<bool> schedulePickup({
    ...
    try {
      final result = await _reservasiService.schedulePickup(
  ...
  /// Start picking up - Driver mulai berangkat
  Future<bool> startPickingUp(String reservasiId) async {
    _setLoading(true);
    _clearError();

    try {
      final result = await _reservasiService.startPickingUp(reservasiId);
  ...
  /// Upload bukti jemput oleh user
  Future<bool> uploadBuktiJemput(String reservasiId, XFile fotoFile) async {
    _setLoading(true);
    _clearError();
    ...
    try {
      ...
      final mimeType = ext == 'png' ? 'image/png' : 'image/jpeg';
      final fotoBuktiJemput = 'data:$mimeType;base64,$base64Image';

      final result = await _reservasiService.uploadBuktiJemput(
        reservasiId: reservasiId,
        fotoBuktiJemput: fotoBuktiJemput,
      );
  ...
  Future<bool> confirmCompleted(String reservasiId) async {
    _setLoading(true);
    _clearError();

    try {
      final result = await _reservasiService.confirmCompleted(reservasiId);
    ...
    try {
      ...
      switch (action) {
        ...
        case 'finish':
          ...
          break;
        case 'expire':
          result = await _reservasiService.updateStatusReservasi(
            reservasiId: reservasiId,
            newStatus: ReservasiStatus.expired,
          );
          break;
        default:
          return false;
