...
class _AdminDashboardPageState extends State<AdminDashboardPage> {
  ...
  Widget _buildStatisticsSection(ReservasiController controller) {
    ...
    final activeRentals = allReservasi.where((r) => r.status == ReservasiStatus.active).toList();
    final totalUnitDisewa = activeRentals.fold<int>(0, (sum, r) => sum + r.jumlahUnit);
    
    // Transaksi yang sudah selesai
    final completedTransactions = allReservasi.where(
      (r) => r.status == ReservasiStatus.finished || r.status == ReservasiStatus.completed
    ).toList();
    
    // Total pendapatan dari transaksi selesai
    final totalPendapatan = completedTransactions.fold<int>(0, (sum, r) => sum + r.totalHarga);
    
    // Transaksi yang menunggu konfirmasi pembayaran (status pending)
    final waitingConfirmation = allReservasi.where(
      (r) => r.status == ReservasiStatus.pending
    ).toList();
  ...
  Widget _buildMapsCard(ReservasiController controller) {
    ...
    final activeWithLocation = controller.reservasiList.where((r) {
      ...
      final isActiveStatus = r.status == ReservasiStatus.active ||
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // Details
            _buildDetailRow(Icons.gamepad, 'Unit', '${reservasi.jumlahUnit} unit × ${reservasi.jumlahHari} hari'),
            _buildDetailRow(Icons.location_on, 'Alamat', reservasi.alamat),
            _buildDetailRow(Icons.phone, 'No. WA', reservasi.noWA),
            _buildDetailRow(Icons.attach_money, 'Total', 'Rp ${_formatCurrency(reservasi.totalHarga)}'),
            
            if (reservasi.status == ReservasiStatus.active && reservasi.waktuSewaBerakhir != null)
              _buildDetailRow(
                Icons.timer, 
                'Sisa Waktu', 
              ...
              children: [
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('Navigasi', style: TextStyle(color: Colors.white)),
